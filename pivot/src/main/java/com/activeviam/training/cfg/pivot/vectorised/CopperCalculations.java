...
public class CopperCalculations {
	...
	protected static Dataset someAggregatedMeasures(final BuildingContext context) {		
		return context
				...
				.agg(
						sum(RISK_BUCKET__VALUE).as("Delta.SUM.INT"),
						sum(TRADES__NOTIONAL).as(NOTIONAL_SUM)
					)
				.withColumn("Delta.SUM", col("Delta.SUM.INT").map((IVector v) -> v.sumDouble()));		
	}
	...
	///			.groupBy(MD_BUCKET__MDBUCKET)				
				.agg(
						sum(MD_BUCKET__VALUE_INTRADAY).as(MD_VALUE_INTRADAY),
						sum(MD_BUCKET__VALUE_SOD).as(MD_VALUE_SOD),
						sum(RISK_BUCKET__VALUE).as(RISK_VALUE_SUM + "LEAF")
					)
				.withColumn(RATE_CHANGE, col(MD_VALUE_SOD).minus(col(MD_VALUE_INTRADAY)))
				.withColumn(RSBPL + ".INT", col(RATE_CHANGE).multiply(col(RISK_VALUE_SUM + "LEAF")))
				.withColumn(RSBPL, col("RSBPL.INT").map((IVector v) -> v.sumDouble()))
				;
	}
